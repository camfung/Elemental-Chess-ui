...
const Timer = ({ initialTime, addedTime, isRunning, addTime }) => {
  const [time, setTime] = useState(initialTime * 60); // Convert minutes to seconds
  const  clockIconClasses = ["clock-left", "clock-up", "clock-right", "clock-down"]
  const [clockIconIndex, setClockIconIndex] = useState(0)

  const clockIconTick = () => {
    let newIndex = clockIconIndex + 1; 
    if (newIndex > 3) 
        newIndex = 0
    setClockIconIndex(newIndex);
  }
  ...
  useEffect(()=> {
    setTime(prevTime => prevTime + (addedTime))
  }, [addTime])
